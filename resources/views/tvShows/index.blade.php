...
<div class="container mx-auto px-4 pt-16 border-b border-gray-800">
    <div class="popular-movies mb-12 ">
        ...
        <div class="grid xs:grid-cols-1 justify-center sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-12">
            ...
            @foreach ($popularTvShows as $AiTvSh)
            <div class="mt-8">
                <a href="{{ route('show.shows', $AiTvSh['id']) }}">
                    <img src="{{ "https://image.tmdb.org/t/p/w500/".$AiTvSh['poster_path'] }}"
                        alt="Move Poster"
                        class="hover:opacity-75 transition ease-in-out">
                </a>
                <div class="mt-2">
                    <a href="{{ route('show.shows', $AiTvSh['id']) }}"
                        class="text-lg mt-2 hover:text-gray-300"> {{ $AiTvSh['name'] }} </a>
                    <div class="flex item-center text-gray-400 mt-1 text-sm">
                        ...
                        <span class="ml-1">{{ $AiTvSh['vote_average' ]* 10 . '%' }}</span>
                        <span class="ml-1">|</span>
                        <span
                            class="ml-1">{{ \Carbon\Carbon::parse($AiTvSh['first_air_date'])->format('M d, Y')  }}</span>
                    </div>
                    @foreach ($AiTvSh['genre_ids'] as $gL)
...
<div class="container mx-auto px-4 pt-16 border-b border-gray-800">
    <div class="popular-movies mb-12 ">
        <h2 class="uppercase tracking-wider text-orange-500 text-lg font-semibold popular-movie">Airing Today</h2>
        <div class="grid xs:grid-cols-1 justify-center sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-12">

            @foreach ($popularTvShowAiring as $AiTvSh)
            <div class="mt-8">
                <a href="{{ route('show.shows', $AiTvSh['id']) }}">
                    <img src="{{ "https://image.tmdb.org/t/p/w500/".$AiTvSh['poster_path'] }}"
                        alt="Move Poster"
